// Тесты собственных flag.Value. Отдельно от flags_test.go по той же причине,
// по которой отдельно values.go: разбор значения меняется, когда меняется его
// правило, а не когда в наборе прибавляется флаг. При том разделении код
// уехал, а тесты остались здесь.
package main
