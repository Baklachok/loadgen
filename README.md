...
**Перцентили — метод ближайшего ранга**: p это наименьшее значение, ниже
которого лежит не менее p% выборки. Всегда время реального запроса, без
интерполяции между соседними.

```go
idx := int(math.Ceil(p*float64(len(sorted)))) - 1
```

Десять замеров по 1..10 мс:
...
На сотне замеров они сходятся, так что сравнивать p99 с `wrk` или `hey`
