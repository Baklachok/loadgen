...
Сервер лежал пятую часть прогона, а closed-loop показал p99 = 16.6 мс: пока сервер стоял, пул стоял тоже и запросов не слал — в статистику попали лишь 20 штук, бывших в полёте. Это coordinated omission, им страдают `ab`, `jmeter` и обычный `wrk`.
...
Без них инструмент нельзя поставить в пайплайн: он всегда «успешен».

| Код | Значение |
|---|---|
| 0 | прогон состоялся — включая случай, когда сервис отдавал одни 500 |
| 1 | ошибка использования: флаги, URL, несовместимые опции |
| 2 | измерить не удалось: ни одного ответа не пришло |
| 3 | зарезервирован под нарушение SLO |
| 130 | прервано по Ctrl+C, результат частичный |

«Сервер вернул 500» — не ошибка loadgen: инструмент существует ровно затем,
чтобы это показать, и краснеть в CI тут нечему. Код 2 означает другое — измерить
не получилось вовсе.
...
На сотне замеров все три сходятся. Поэтому сравнивать p99 с `wrk` или `hey`
осмысленно только на больших выборках — на малых loadgen его и не печатает.
Метод не меняется между версиями, иначе сравнение «до и после» ничего не значит.
