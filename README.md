...
`p99` сверяется с поправкой на расписание; в долю ошибок входят не-2xx.
Не хватило замеров на p99 — код 2, а не 0.

### JSON

Первое поле — `schema`: меняется, когда поле убрано, переименовано или сменило
смысл. Проверяйте и падайте на незнакомой. Длительности в миллисекундах. Блоки `corrected` и `max_lag_ms` появляются только
...
то же один раз — для Pushgateway или textfile-collector.
...
флагов — HTTP-семантика, и она же зафиксирована в JSON-контракте. gRPC — другой
транспорт с другой моделью успеха и без цели без схемы; для него есть `ghz`.
`loadgen grpc://…` скажет это прямо, а не «без схемы».
