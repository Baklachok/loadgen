...
Сервер лежал пятую часть прогона, а closed-loop показал p99 = 16.6 мс: пока он стоял, пул стоял тоже и запросов не слал — в статистику попали лишь те 20, что были в полёте. Это coordinated omission, им страдают `ab`, `jmeter` и обычный `wrk`.
...
| `-rate` | 0 | постоянный RPS, включает open-loop; не выше 1e9 |
| `-m` | GET | HTTP-метод |
| `-d` | — | тело запроса |
...
| `-slo-error-rate` | — | порог приёмки: доля не-2xx в процентах, 0–100 |
...
`loadgen -f run.yaml`. Ключ — имя флага или его длинная форма; список —
только у `headers`. Блок `config` из JSON-отчёта переписывается в такой
файл — прогон повторяется.
...
`-regress-p99` и `-regress-rps` — в процентах, явный `0` — ни на процент;
нарушение даёт код 3.
...
(+<репозиторий>)`; перебивается своим `-H 'User-Agent: ...'`.
...
На восьмиядерном i5-8250U — **~24 000 RPS**. **Всё, что в реальном тесте близко
к вашему числу, измеряет loadgen, а не сервис.**
